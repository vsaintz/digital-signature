  ...
  template: `
    @if (visible()) {
      ...
      >
        ...
        >
          <h2 class="text-[15px] font-semibold text-foreground mb-1.5">{{ title }}</h2>
          <p class="text-[13px] text-foreground-muted leading-relaxed mb-6">{{ description }}</p>
...
export class DeleteConfirmDialogComponent implements OnChanges {
  ...
  get description(): string {
    if (this.deleteCount === 1 && this.documentName) {
      return `${this.documentName} will be permanently deleted and cannot be recovered.`
    }
    if (this.deleteCount === 1) {
      return "This document will be permanently deleted and cannot be recovered."
    }
    return `${this.deleteCount} documents will be permanently deleted and cannot be recovered.`
  }
